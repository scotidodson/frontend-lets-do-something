...
class EventDetails extends Component {
    ...
    switch (userIsHost) {
      case true:
        if (thisEvent.winner) {
          ...
          this.renderActivePollForHost(thisEvent)
        }
      ...
      case false:
        if (thisGuest.rsvp === "yes") {
          if (thisEvent.winner) {
            ...
            this.renderActivePollForGuest(thisEvent)
          }
        } else if (thisGuest.rsvp === "no") {
          this.renderSaidNo(thisEvent)

        } else {
          if (thisEvent.winner) {
            this.renderRsvpChangePage(thisEvent)
          } else {
            this.renderVotePage(thisEvent)
          }
        }
        break;
      default:
  ...
  renderActivePollForHost = (thisEvent) => {
    // current results poll happening
    // include cancel button

  }

  renderActivePollForGuest = (thisEvent) => {
    // current results poll happening
